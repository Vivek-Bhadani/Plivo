...
# def normalize_email_tokens(s: str) -> str:
#     s = re.sub(r'(\s|dot)+com\b', '.com', s, flags=re.IGNORECASE)
...
#         else:
#             if buf:
#                 out.append(words_to_digits(buf))
#                 buf = []
#             out.append(t)
#     if buf:
#         out.append(words_to_digits(buf))
...
#         s = str(n)
#         if len(s) <= 3: return s
#         return f"{s[:-3][:-2]},{s[:-3][-2:]},{s[-3:]}" if len(s[:-3]) > 2 else f"{s[:-3]},{s[-3:]}"
#     def repl(m):
#         digits = re.sub(r'\D', '', m.group(0))
#         if not digits: return m.group(0)
#         return '₹' + group_indian(digits)
...
#     toks, out = s.split(), []
#     for t in toks:
#         best = process.extractOne(t, names_lex, scorer=fuzz.token_sort_ratio)
#         out.append(best[0] if best and best[1] >= threshold else t)
#     return ' '.join(out)
...
# def generate_candidates(text: str, names_lex: List[str]) -> List[str]:
#     cands = set()
#     base = add_minimal_punctuation(
#         normalize_currency(
#             normalize_numbers_spoken(
#                 normalize_email_tokens(
#                     correct_names_with_lexicon(text, names_lex)
#                 )
#             )
#         )
#     )
...
#     return list(sorted(cands, key=len))[:5]

# # ---------- Ablation / Heuristic Summary ----------
# # • Fixed email normalization for “at”, “dot”, spaces, and “dot com”
# # • Handled “double/triple” and “oh” in spoken numbers
# # • Added ₹ + Indian digit grouping (e.g. 145000 → ₹1,45,000)
# # • Fuzzy name correction threshold tuned to 88 for Indian names
# # • Added minimal punctuation (final period, proper spacing)
# # • Limited candidates to ≤5 to preserve latency <30 ms
